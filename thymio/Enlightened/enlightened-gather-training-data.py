...
def mainLoop(robot):
    ...
    while (time_step > 0):
        ...
        left, right = random.choice(action_space)
        robot.drive(left, right)

        s0,s1,s2,s3,s4,s5,s6 = robot.sens_horizontal() #0-4: Front left to right, 5-6: Back left, right
        s7,s8 = robot.sens_vertical() #7-8: Bottom left, right
        row = [s0,s1,s2,s3,s4,s5,s6,s7,s8, left, right]
